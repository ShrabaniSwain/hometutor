...
import com.flyngener.hometutor.Constant.PROFILEID
import com.flyngener.hometutor.databinding.FragmentGuardianHomeBinding
            ...
            override fun onResponse(call: Call< ServiceProfileGuardianResult>, response: Response<ServiceProfileGuardianResult>) {
                if (response.isSuccessful) {
                    ...
                    response.body()?.result?.let { featuredServices ->
                        ...
                        val filteredList = featuredServices.filter { it.service_profile_type == "Featured" }
                        ...
                        binding.rvClass.layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.VERTICAL, false)
                    ...
                    binding.moreTeacherCard.visibility = if (binding.biologyTeacherCard.visibility == View.VISIBLE) View.VISIBLE else View.GONE
        ...
        call.enqueue(object : Callback<PopularTeacherResponse> {
            override fun onResponse(call: Call< PopularTeacherResponse>, response: Response<PopularTeacherResponse>) {
                if (response.isSuccessful) {
                    ...
                    val classServicesAdapter = ClassServicesAdapter(requireContext(), teacherList)
                    binding.rvServices.adapter = classServicesAdapter
                    binding.rvServices.layoutManager = GridLayoutManager(requireContext(), 2, GridLayoutManager.VERTICAL, false)
                    Log.i("TAG", "responseList: ${response.body()}")
                } else {
                    hideProgressBar()
                    val errorBody = response.errorBody()
